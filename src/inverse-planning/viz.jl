...
function draw_start(scene::Scene, start::Point, target_frame=luxor_frame(DEFAULT_IMAGE_WIDTH, DEFAULT_IMAGE_HEIGHT); markersize=8, startopacity=1)
    ...
    Luxor.setcolor(Luxor.sethue("purple")..., startopacity)
    Luxor.star(convert(start), markersize, 2, 1, 0, :fill) 
    # Luxor.circle(convert(start), markersize, :stroke)
    p = convert(start)
    p = Point(p.x, p.y+25)
    Luxor.textcentered("START", p)
end
...
function draw_dest(scene::Scene, dest::Point, target_frame=luxor_frame(DEFAULT_IMAGE_WIDTH, DEFAULT_IMAGE_HEIGHT); markersize=8, destopacity=1)
    convert = frame_converter(Frame(scene), target_frame)
    Luxor.setcolor(Luxor.sethue("purple")..., destopacity)
    Luxor.star(convert(dest), markersize, 2, 1, 0, :fill) 
    # Luxor.circle(convert(dest), markersize, :stroke)
    p = convert(dest)
    p = Point(p.x, p.y+25)
    Luxor.textcentered("END", p)
end
...
function trace_to_dict(trace)
    args = Gen.get_args(trace)[1]
    scene = args[:scene]
    num_ticks = args[:num_ticks]
    choices = Gen.get_choices(trace)
    result = Gen.get_retval(trace)
    planning_failed, maybe_path = result[:planning_failed], result[:maybe_path]
...
function visualize_grid(element_drawer, elements, num_columns, total_width=DEFAULT_IMAGE_WIDTH, element_aspect_ratio=1.0; separators=nothing)
    ...
    Luxor.@png draw_grid(element_drawer, elements, num_columns, total_width, element_aspect_ratio; separators=separators) total_width total_height
end
