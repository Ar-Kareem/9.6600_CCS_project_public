...
function nearest_node_on_tree(tree::RRT, conf::Point)
    ...
    best_dist::Float64 = distsq(nearest.conf, conf)
    for node in tree.nodes
        d = distsq(node.conf, conf)
        if d < best_dist
            best_dist = d 
            nearest = node
        end
...
function generate_rrt(scene::Scene, init::Point, iters::Int, dt::Float64; tree=nothing)
    if isnothing(tree)
        tree = RRT(init)
        new_node = nothing
    else
    ...
    for iter=1:iters
        if iter==1 && !isnothing(new_node)
            rand_conf::Point = new_node
        else
            rand_conf = random_point_in_scene(scene)
        end
...
function get_path_to_dest(tree::RRT, destination::Point, scene::Scene)
...
function get_path_from_to(tree::RRT, p1::Point, p2::Point, scene::Scene)
    path_p1 = get_path_to_dest(tree, p1, scene)
    path_p2 = get_path_to_dest(tree, p2, scene)
    if isnothing(path_p1) || isnothing(path_p2)
        return nothing
    end
    
    result = nothing
    path_p1 = reverse(path_p1.points)  # start at p1 -> ... -> root
    path_p2 = reverse(path_p2.points)  # start at p2 -> ... -> root
    for p1_i in 1:length(path_p1)
        for p2_i in 1:length(path_p2)
            clear_path = !line_is_obstructed(scene, path_p1[p1_i], path_p2[p2_i])
            if clear_path
                # concat the two paths at the point where clear line of sight
                result = Path([path_p1[1:p1_i]; reverse(path_p2[1:p2_i])])
                break
            end
        end
        if !isnothing(result)
...
function plan_path(start::Point, dest::Point, scene::Scene, params::PlannerParams)
    ...
    return tree_refined_dest_path(tree, dest, scene, params)
end

function tree_refined_dest_path(tree::RRT, dest::Point, scene::Scene, params::PlannerParams)
    # Find a route from the root of the tree to a node on the tree that has a line-of-sight to the destination
    maybe_path = get_path_to_dest(tree, dest, scene)
